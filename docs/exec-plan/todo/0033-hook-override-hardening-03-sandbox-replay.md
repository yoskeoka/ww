# 0033: Hook Override Hardening 03 Sandbox Replay
...
- This plan should reuse the effective config and lifecycle work from `0031` and `0032` instead of inventing a second hook/materialization engine.
...
- [ ] Extract reusable post-create materialization logic that can:
  ...
  - reuse the target repo's effective config from `0031`
  - reuse lifecycle hook execution from `0032`
...
- Depends on `0031-hook-override-hardening-01-child-repo-overrides.md`.
- Depends on `0032-hook-override-hardening-02-lifecycle-context.md`.
