...
- Commands load configuration from the user-owned global config path first, then overlay any repo-local `.ww.toml` discovered for the active repository. When `--repo <name>` selects a workspace child repository, that child repository becomes the active repository for config discovery and project matching. Repo-local values replace same-key global values completely, including arrays and hook strings.
...
| `merged` | Branch changes are already integrated into the resolved base branch by direct merge, by commit-equivalent rebase/cherry-pick, or by a squash merge that lands the branch diff as a single base commit |
...
This includes worktrees whose branches were integrated into the resolved base branch according to the `merged` status contract above, even if the local branch has no tracking remote.
