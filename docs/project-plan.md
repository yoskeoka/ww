...
- **FR-27** (`implemented`): Global config file support — `ww` loads user-owned global configuration from `$XDG_CONFIG_HOME/ww/config.toml` when set, otherwise from the current user's default home config location for `ww`, so teams or individuals can avoid committing `.ww.toml` into every target repository.
- **FR-28** (`implemented`): Global project-target matching — `ww` global config can select per-project settings with ordered `[[projects]]` blocks matched against the repository main worktree root, so similar repository layouts or workspace members can share one centrally maintained config set without committed repo-local config.
- **FR-29** (`implemented`): Hook-driven materialization profiles — global config can define reusable copy/symlink/setup profiles and select one per config layer, so sandbox-friendly worktree setup patterns such as linking `.env`, dependency directories, or tool caches can be reused without repeating the same materialization payload in every repository.
- **FR-30** (`implemented`): Safe global-config guidance for sandboxed agents — documented recommended permission patterns for the `ww` global config location, including both `$XDG_CONFIG_HOME/ww/*` and the default home-config location (for example, allow reads but deny writes for agent sandboxes), so global hook/config workflows reduce prompt-injection and config-tampering risk.
...
- [x] Phase 5: Global config and hook workflow portability — add user-owned global config, per-project target matching, reusable hook/materialization profiles, and safe sandbox guidance so agent workflows do not require committing `.ww.toml` into every repository.
