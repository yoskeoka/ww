...
- **FR-27** (`implemented`): Global config file support — `ww` loads user-owned global configuration from `$HOME/.config/ww/config.toml` or `$XDG_CONFIG_HOME/ww/config.toml` so teams or individuals can avoid committing `.ww.toml` into every target repository.
...
- **FR-30** (`implemented`): Safe global-config guidance for sandboxed agents — documented recommended permission patterns for `$HOME/.config/ww/*` or `$XDG_CONFIG_HOME/ww/*` (for example, allow reads but deny writes for agent sandboxes) so global hook/config workflows reduce prompt-injection and config-tampering risk.
